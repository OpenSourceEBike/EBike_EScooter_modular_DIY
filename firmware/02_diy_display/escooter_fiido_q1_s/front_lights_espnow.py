...
# MicroPython ESP-NOW helper for the front lights (synchronous espnow).
...
class FrontLights:
    """
    Send front-lights state over ESP-NOW (synchronous version).

    Args:
        espnow_inst (espnow.ESPNow): active instance (espnow_inst.active(True) done by caller)
        peer_mac (bytes): 6-byte MAC of the receiver (front lights board)
    ...
    def _build_payload(self) -> bytes:
        pins_state = getattr(self._system_data, "front_lights_board_pins_state", 0)
