...
class Display(object):
    ...
    def _send_data(self):
        ...
        # tx_array[1] - this is the lenght byte
        _len = 2 # start byte + len byte

        data_pack_offset = 1
        struct.pack_into('<H', tx_array, 2, data_pack_offset + int(self._ebike_data.battery_voltage * 100))
        tx_array[4] = data_pack_offset + int(self._ebike_data.battery_current * 5)
        struct.pack_into('<H', tx_array, 5, data_pack_offset + int(self._ebike_data.motor_power))
        struct.pack_into('<H', tx_array, 7, data_pack_offset + int(self._ebike_data.vesc_temperature_x10))
        struct.pack_into('<H', tx_array, 9, data_pack_offset + int(self._ebike_data.motor_temperature_sensor_x10))
        tx_array[11] = data_pack_offset + self._ebike_data.vesc_fault_code
        tx_array[12] = data_pack_offset + self._ebike_data.brakes_are_active
